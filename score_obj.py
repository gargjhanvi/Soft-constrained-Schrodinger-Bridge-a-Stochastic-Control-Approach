...
from losses import dsm_score_estimation_obj
import argparse
...
num_workers_s =  config["training"]["num_workers"]
...
dataloader_obj = DataLoader(dataset_obj, batch_size=batch, shuffle=True, num_workers=num_workers_s)
dataloader_ref = DataLoader(dataset_ref, batch_size=batch, shuffle=True, num_workers=num_workers_s)
...
def train_obj(beta):
    ...
    for epoch in range(n_epochs):
        for (obj_data, ref_data) in zip(dataloader_obj,dataloader_ref):
            ...
            lossa = dsm_score_estimation_obj(score,D, beta, X, labels, sigmas) 
            loss = lossa
            optimizer.zero_grad()
            loss.backward()
            print(step)
            ...
            if step % snapshot_freq== 0:
                ...
                torch.save(states, os.path.join(os.getcwd(), "Model","score_obj", 'checkpoint_{}.pth'.format(step)))
                torch.save(states, os.path.join(os.getcwd(), "Model","score_obj", 'checkpoint.pth'))
